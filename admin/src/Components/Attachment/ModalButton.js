            ...
            <div className="media-bottom-btn">
                <div className="left-part">
                    <div className="file-detail">
                        <h6>{state.selectedImage?.length || 0} {"FileSelected"}</h6>
                        <a href="#" className="font-red" onClick={() => dispatch({ type: "SELECTEDIMAGE", payload: [] })}>{"Clear"}</a>
                    </div>
                </div>
                <div className="right-part">
                    <Btn type="submit" className="btn btn-solid" title={tabNav === 2 ? "Submit" : "InsertMedia"} onClick={() => handleClick(state.selectedImage)} />
                </div>
            </div>
        </>
    );
