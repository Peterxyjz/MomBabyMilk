      ...
      {props.label ? (
        <>
          <FormGroup floating>
            ...
            {touched[fields.name] && errors[fields.name] ? <FormFeedback>{handleModifier(errors[fields.name]).split(" ").join("")}</FormFeedback> : ""}
          </FormGroup>
        </>
      )
        : props.inputaddon ? (
          <>
            <InputGroup>
              ...
              {touched[fields.name] && errors[fields.name] ? <FormFeedback>{handleModifier(errors[fields.name]).split(" ").join("")}</FormFeedback> : ""}
              {props?.errormsg && <ErrorMessage name={fields.name} render={(msg) => <div className="invalid-feedback d-block">{props.errormsg} {'IsRequired'}</div>} />}
            </InputGroup>
            ...
            <>
              {
                props.type == "color" ? <div className="color-box">
                  ...
                  {touched[fields.name] && errors[fields.name] ? <FormFeedback>{handleModifier(errors[fields.name])}</FormFeedback> : ""}
                  <h6>{fields.value}</h6>
                </div>
                  :
                  <>
