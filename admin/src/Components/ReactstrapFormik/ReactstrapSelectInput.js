      ...
      <div className={`custom-select-box ${props.formfloat=="true"&&'form-floating'}`} ref={ref}>
        {Array.isArray(selectedItems) ?
          ...
          <Input type="text" className="form-control form-select cursor position-absolute"
            value={setvalue !== undefined ? (props.inputprops.value || selectedItems?.name) : props.inputprops?.options?.find((item) => item.id === field.value)?.name || "Select"}
              ...
              <Fragment key={index}>
                {option?.data ?
                  ...
                  : <li onClick={() => onSelectValue(option)}>
                    ...
                    />}
                    <p className={`cursor ${(selectedItems?.[index]?.[getValuesKey] || selectedItems?.[getValuesKey]) == option[getValuesKey] ? 'selected' : ""}`}>{option.name}</p>
                  </li>}
              </Fragment>
            ))}
          </ul>
        ...
        {touch && error && <FormFeedback>{t(props.title) || t(props?.floatlabel)} {"không được để trống"}</FormFeedback>}
        {props?.inputprops?.close && <div className="close-icon"><RiCloseLine onClick={() => RemoveSelectedItem()} /></div>}
       { props?.floatlabel&&<Label>{props?.floatlabel}</Label>}
      </div>
    </>
  );
};
