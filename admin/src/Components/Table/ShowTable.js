      ...
      <thead>
        <tr>
          <>
            ...
            {headerData.isSerialNo !== false && <th className="sm-width">{"No"}</th>}
            {/* Table Heading */}
            ...
            {headerData?.isOption && <th>{headerData?.optionHead?.title}</th>}
          </>
        </tr>
      </thead>
      <tbody>
                ...
                {headerData?.column.map((item, i) => (
                  <td className={item.type == "image" ? "sm-width" : ""} key={i} onClick={(e) => item.type !== "switch" && isHandelEdit(e, tableData, headerData)}>
                    {item.type == "date" ? (
                      ...
                      <>{ headerData?.data?.[index].system_reserve == "1" ? <Status data={tableData} url={url} disabled={true} /> : <Status data={tableData} url={item.url ? item.url : url} apiKey={item.url && item.apiKey} />}</>
